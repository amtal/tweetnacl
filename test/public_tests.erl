...
prop_enc_dec() ->
    ?FORALL(M, binary(),
        ?FORALL(N, binary(24),
        begin
                {PKA,SKA} = tweetnacl:box_keypair(), % agent A
                {PKB,SKB} = tweetnacl:box_keypair(), % agent B
                C = tweetnacl:box(M, N, PKB, SKA), % A sends to B
...
prop_precomp_enc_dec() ->
    ?FORALL(M, binary(),
        ...
        begin
                ...
                C = tweetnacl:box_afternm(M, N, KA), % A sends to B
                {ok,M2} = tweetnacl:box_open_afternm(C, N, KB), % B receives from A
                (KA == KB) and (M==M2)
        end)).

prop_keygen() ->
    {_,_} = tweetnacl:box_keypair(),
    true.
